...
static void MODE0_1Setup() { //smart mode start sequence
  while (digitalRead(SELECT_PIN) != HIGH) {
      ...
      Serial.print("ACTIVATE_TIME: ");
      Serial.print(ACTIVATE_TIME/60); Serial.print(":");Serial.println(ACTIVATE_TIME%60);
...
static bool shouldItRain(){  
  ...
  float daysSinceLastRain = 7;//timeSinceLastRain/1440;
  float inchesNeeded = INCHES_WEEK - SENSOR_INCHES;
  float WeightedForecast = 1;//((Day1Rain * 4) + (Day2Rain * 3) + (Day3Rain * 2) + (Day4Rain * .75) + (Day5Rain * .25))/10;
...
void loop() {
  ...
  //while (digitalRead(SELECT_PIN) != HIGH) {
    ...
    if ((MODE == 0) && (Setup == false)) {
      Serial.println("MODE0 Ran");
      delay(1500);
      MODE0_1Setup();
    } if ((MODE ==1) && (Setup == false)) {
      Serial.println("MODE1 Ran");
      delay(1500);
      MODE0_1Setup();
    } if ((MODE == 2) && (Setup == false)) {
      Serial.println("MODE2 Ran");
      delay(1500);
      MODE2Setup();
    }
  ...
  if ((MODE == 0) && (Setup == true)) {
    //control sprinklers all complex-like. idle loop for when mode = 0.
    //delay(500); //ADD 2 more 0s for 3 minute delay
    getTime();
    ...
    //if (((ACTIVATE_TIME-5) < TimeInMinutes) && ((ACTIVATE_TIME+5) > TimeInMinutes)) {
      if (shouldItRain() == true) {
        Serial.println(shouldItRain());
        digitalWrite(OUTPUT_PIN, HIGH);
        Serial.println("OUTPUT_PIN RAN");
      }
    //}
  ...
  if ((MODE == 1) && (Setup == true)) { ///WHEN SPRINKLER RUNS, ADD AMNT OF WATER TO SENSOR MEASUREMENT
    getTime();
    if (((ACTIVATE_TIME-5) < TimeInMinutes) && ((ACTIVATE_TIME+5) > TimeInMinutes)) {
